...
// User Accounts
let deployerWallet, buyerWallet, sellerWallet;

if (deployment.network === "sepolia") {
  deployerWallet = new ethers.NonceManager(new ethers.Wallet(process.env.DEPLOYER_PRIVATE_KEY, provider));
  buyerWallet = new ethers.NonceManager(new ethers.Wallet(process.env.BUYER_PRIVATE_KEY, provider));
  sellerWallet = new ethers.NonceManager(new ethers.Wallet(process.env.SELLER_PRIVATE_KEY, provider));
} else {
  ...
  deployerWallet = new ethers.NonceManager(new ethers.Wallet(HARDHAT_PRIVATE_KEYS[0], provider));
  buyerWallet = new ethers.NonceManager(new ethers.Wallet(HARDHAT_PRIVATE_KEYS[1], provider));
  sellerWallet = new ethers.NonceManager(new ethers.Wallet(HARDHAT_PRIVATE_KEYS[2], provider));
}
